...
public:
    ...
    static void setClockAtTime(Clock *c, double pts, double time);
private:
    ...
    Clock *master_clock;
    static uint32_t hw_bytes_per_sec; //音频设备硬件缓冲区大小
    static uint32_t bytes_per_sec; //每一帧音频大小
    static uint64_t audio_callback_time; //音频设备回调时间
